...
where in practice we are usually interested in one extreme (high cost, low capacity and so forth).

For the classic knapsack problem one can bait a naive solver to pick a high-deviation / high-value item A $\bar{c}_A = 100, d_A = 90$
as compared to a safer option B $\bar{c}_B = 60, d_B = 2$.
With Bertsimas-Sim constraints and a reasonable $\Gamma$ budget the solver will prioritize B over A.
